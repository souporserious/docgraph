...
import alias from "esbuild-plugin-resolve"
...
export async function parseMDX(path: string, contents: string) {
  try {
    ...
    if (highlighter === null) {
      highlighter = await getHighlighter(resolve(process.cwd(), "theme/code.json"))
    }
...
async function transformReadme(path) {
  ...
  const result = await esbuild.build({
    ...
    // external: ["react", "react-dom", "@mdx-js/react", "components"],
  })
