...
writeData().then(() => {
  if (DEBUG) {
    console.log(`finished gathering data in ${((performance.now() - start) / 1000).toFixed(2)}s`)
  }
})

if (process.argv.includes("--watch")) {
  const watcher = chokidar.watch([
    ...docsSourceFiles.map((file) => file.getFilePath()),
  ...
  watcher.on("change", async function (changedPath) {
    ...
    if (DEBUG) {
      console.log(`finished updating data in ${((performance.now() - start) / 1000).toFixed(2)}s`)
    }
  })
}
